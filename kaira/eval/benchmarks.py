...
class BenchmarkRunner:
    ...
    def export(self, output_dir: str | Path) -> EvalResult:
        ...
        metrics = build_metrics(responses, scenario_results)
        result = EvalResult(run_id=str(uuid.uuid4()), scenario_count=len(scenario_results), metrics=metrics, scenarios=scenario_results)
        ...
        reporter = MetricsReporter(output_path)
        reporter.write_summary(result)
        reporter.write_per_case(result)
        reporter.write_metrics_table(result)
        reporter.write_category_summary(result)
        reporter.write_latency_summary(result)
        reporter.write_operational_traces(result)
        reporter.write_sample_traces(responses)
        return result
